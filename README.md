...
- OpenGL ES command execution is accelerated through virpipe and Android EGL/GLES with software fallback. Final Wayland presentation still uses shared-memory copies; Vulkan, zero-copy dmabuf, and physical-device end-to-end app tests remain incomplete. Pulse playback through Android AAudio is validated with unmodified `pavucontrol` on x86_64 and AArch64; microphone capture, printing, camera, drag-and-drop, accessibility, keyrings, and many desktop portals remain absent.
...
2. Complete the remaining Android capability brokers for microphone capture, printing, camera, drag-and-drop, accessibility, and secrets/keyrings. Manager-owned GUI homes, active-app document activation, private XDG URL/notification adapters, and Pulse-to-AAudio playback are implemented.
