...
- Verified package closures publish as manager-owned immutable content-addressed runtime packs. A caller-authenticated provider grants exact read-only modules to the generated wrapper UID, cold KCalc app-drawer launch is validated, untrusted shell access is rejected, unchanged closures are reused, and external uninstalls are reconciled per package. ARM artifacts support 4 KB/16 KB pages; upstream Arch x86_64 is 4 KB-only. Stable provider clients and Binder-death leases now protect packs used by running wrappers and Terminal materialization; complete process-tree cleanup remains incomplete.
...
2. Add complete process-group cleanup to the package-derived content-addressed runtime-pack broker.
