...
- OpenGL ES command execution is accelerated through virpipe and Android EGL/GLES with software fallback. Complete GLMark2 runs pass on the x86_64 emulator and physical AArch64 Samsung; final Wayland presentation still uses shared-memory copies, while Vulkan, zero-copy dmabuf, and broader application coverage remain incomplete. Unexpected helper loss replaces the same-UID virgl helper and restarts the Linux payload once through virpipe without losing the Android Activity; bounded llvmpipe recovery remains only if helper replacement cannot recover. The current Rust/Kotlin path validates private Pulse playback and microphone input through Android AAudio with unmodified `pavucontrol` on x86_64 and AArch64. Microphone capture is a separate reviewed capability, begins only after a real recording stream and explicit manager consent, runs under Android's microphone foreground-service and privacy rules, and stops with the authenticated launcher session. XDG printing reaches Android's system print UI, and bounded plain-text plus brokered file drag-and-drop maps to standard Wayland data devices. A bounded explicit-consent Camera2 JPEG API is validated on x86_64 and AArch64. The private AT-SPI2 adapter maps unmodified KCalc/Qt and Mousepad/GTK semantics and reverse actions on x86_64 and AArch64. The current generated-wrapper path validates a per-wrapper Android Keystore-backed encrypted collection and private Secret Service D-Bus adapter on both architectures with unmodified Arch `secret-tool`; isolated KWallet fixtures retain broader client evidence. Streaming XDG Camera/PipeWire is validated with unmodified Snapshot on x86_64 and AArch64; Vulkan, zero-copy presentation, and many desktop portals remain incomplete.
