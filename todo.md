...
- [ ] Add a bounded AUR workflow suitable for packages such as `visual-studio-code-bin`.
  ...
  - [x] Resolve one bounded official build-environment transaction from the complete `base-devel` package plus every reviewed AUR `makedepends` and `checkdepends`, stripping only validated version operators and failing closed when a target is unavailable from the official repositories. Resolve against an ephemeral manager-owned database with current sync catalogs and no installed-package state so an empty Builder root receives the complete closure. The current Code AArch64 candidate requires 152 official packages and 224,514,136 archive bytes.
  - [x] Download every exact build-environment archive and detached signature into the manager cache, require the pinned architecture signer plus exact package name/version/architecture metadata, retain the exact Rust resolution, and reverify the complete closure before reporting success. A Samsung cache-reuse gate reverified all 152 Code build packages, removed its ephemeral resolution database, kept Install disabled and all 36 shared-root pacman entries unchanged, and visibly reports the verified 215 MiB closure in a full-device capture.
  - [x] Canonicalize the verified closure as a bounded Rust manifest containing every exact repository/name/version/URL, archive size/SHA-256, and detached-signature size/SHA-256; validate it against the retained resolution in Kotlin, expose its overall SHA-256 for approval/provenance, and permit descriptor reopening only while that retained closure remains verified. The Samsung gate parses all 152 entries and renders the closure digest.
  - [x] Transfer the retained closure to the separate Builder UID in bounded eight-package Binder batches. Rust duplicates the read-only descriptors, uses no-follow directory-FD operations to reset and populate Builder-private storage, streams through fixed scratch buffers, rehashes every archive/signature pair, and atomically publishes the exact manifest only after complete reverification. Host traversal/symlink tests and the Samsung gate cover all 152 pairs, the 224,514,136 archive bytes, the matching whole-manifest digest, and unchanged 36-entry shared pacman database.
  - [x] Provision a fresh Builder-private Arch root from the exact published closure without invoking Android host extractors. Rust scans every signed XZ/Zstandard package before mutation, rejects escapes and unsupported entries, rehashes each archive again before extraction, resets more than the former 4,096-entry cleanup limit through no-follow directory-FD traversal, materializes hard links that Android app storage forbids, syncs the filesystem, and atomically publishes a closure-bound root manifest. Host hostile-path/recovery tests and a failed-then-recovered Samsung gate prove all 152 packages, 48,271 archive entries, 1,221,416,416 expanded bytes, executable `bash`/`makepkg`/`fakeroot`, unchanged 36-entry shared pacman state, and a full-device review capture.
  ...
  - [ ] Provision a minimal verified Arch build root in the Builder's private storage, execute the approved build without Android network permission, and return verifiable package output through a manager-owned descriptor.
    - [ ] Keep every reusable Builder workspace behind no-follow directory-FD/openat-style Rust operations, kill and reap every prior Builder descendant, and treat all Builder-writable output as hostile after each recipe.
      ...
      - [ ] Safely enumerate hostile post-build output, reject links/substitutions and unexpected archives, verify metadata/provenance in Rust, and copy only a verified package through a manager-owned descriptor.
  ...
  - [ ] Run builds as the separate unprivileged Builder UID and clearly communicate that installed Arch/AUR packages join the shared Linux trust domain after manager verification and installation.
...
- [ ] Perform a complete UX pass over discovery, package details, installation, launcher creation, updates, storage, permissions, setup, settings, and recovery.
  ...
  - [ ] Collapse or dismiss terminal completed activity cards so a prior transaction cannot obscure most of a phone-sized package/AUR review; the current Samsung build-root capture still shows the retained `ttf-dejavu` completion card consuming the lower review surface.
  ...
  - [ ] Replace the raw multiline AUR build transcript with a compact phase card, expandable/copyable logs, verified output summary, and clear next action. The successful Samsung Code build currently fills most of the phone viewport with makepkg output and says “verifying output next” before product-side output verification exists.
