...
- [ ] Make every install/update/remove operation appear in the app list immediately with persistent state and progress.
  ...
  - [ ] Prove a live newly queued operation appears in its matching or appended package row before the worker advances, then complete retry/recovery actions; the code now publishes job revisions and appends unmatched active jobs, but the deterministic device gate currently covers a matching durable Failed row.
