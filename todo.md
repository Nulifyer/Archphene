...
- [ ] Make every install/update/remove operation appear in the app list immediately with persistent state and progress.
  ...
  - [ ] Build the structured available-package list and merge a newly queued operation into the relevant package row immediately; the durable recent-activity card remains visible independently.
...
- [ ] Perform a complete UX pass over discovery, package details, installation, launcher creation, updates, storage, permissions, setup, settings, and recovery.
  ...
  - [ ] Audit populated available search/details, every live durable transaction phase, retry/recovery, and an active terminal in the new navigation.
