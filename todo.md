...
- [ ] Add capabilities in test-gated vertical slices; do not port code merely because it existed before.
  ...
  - [ ] Terminal/PTY and shared command environment
    ...
    - [ ] Build the conventional interactive shared shell environment, locale policy, startup files, and user-selected command/path behavior without escaping into Android's `/system/bin`.
      ...
      - [ ] Add reviewed startup adapters for additional installed shells.
      - [x] Expose only `.bashrc` and `.bash_profile` through a read/write, non-creating virtual DocumentsProvider folder backed by Rust no-follow opens; keep all other dotfiles private. Create `~/.local/bin` safely and put it first in the shared process PATH so user commands work consistently in terminals and graphical apps. Host safety/lint gates and real Android Files workflows pass on both exact-ABI targets.
    ...
    - [ ] Replace the diagnostic command panel with production terminal rendering, scrollback, selection, IME, hardware keyboard, clipboard, and accessibility.
      ...
      - [ ] Complete remaining xterm controls/modes and richer terminal accessibility before calling the surface production-ready.
...
- [ ] Add a bounded AUR workflow suitable for packages such as `visual-studio-code-bin`.
  ...
  - [x] Resolve split-package RPC results through their canonical package-base snapshot, retain the exact requested split identity, recognize both hyphenated and normalized package functions, and carry SHA-512 declarations through native cache verification and the v3 review wire while deriving independent SHA-256 Builder evidence. The live current `dotnet-sdk-bin` review and its 225 MiB ARM64 SDK source pass on Samsung with full-device evidence.
  ...
  - [x] Provision a minimal verified Arch build root in the Builder's private storage, execute the approved build without Android network permission, and return verifiable package output through a manager-owned descriptor.
    ...
    - [ ] Report distinct download, closure staging, root scan/extraction, recipe build, and output verification progress.
      ...
      - [ ] Make the initial whole-closure scan granular/cancellable and add descriptor-copy/output-verification progress.
  ...
  - [x] Remove bounded regular transient Builder-output copies on manager startup when their in-memory verified capability cannot survive process death. A physical restart reclaimed six stale outputs and reduced the manager cache from 1.2 GiB to 3.5 KiB without touching the installed package or durable content-addressed cache.
  ...
  - [ ] Persist the verified built-package capability across manager process death, define recursive AUR dependency behavior, and finish the install-script/scriptlet policy.
...
- [ ] Open the project in Code and validate editing, search, Git, terminal PTY, language services, restore/build, and extension-host subprocesses.
  ...
  - [ ] Install and validate `git` through Archphene's normal package UI. The integrated terminal currently reports `git: command not found`; do not add it outside the shared package model or without user approval.
  ...
  - [ ] Give Code a usable phone-first workspace layout without application patches. Its default Explorer + editor + Chat arrangement leaves the editor only a few characters wide; collapsing both standard sidebars makes editing usable, but Archphene needs a documented or generic remembered phone policy.
