...
- [ ] Add a bounded AUR workflow suitable for packages such as `visual-studio-code-bin`.
  ...
  - [ ] Provision a minimal verified Arch build root in the Builder's private storage, transfer only reviewed inputs through bounded descriptors, execute the approved build without Android network permission, and return verifiable package output through a manager-owned descriptor.
