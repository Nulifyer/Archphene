...
- [ ] Add a bounded AUR workflow suitable for packages such as `visual-studio-code-bin`.
  ...
  - [x] Resolve one bounded official build-environment transaction from the complete `base-devel` package plus every reviewed AUR `makedepends` and `checkdepends`, stripping only validated version operators and failing closed when a target is unavailable from the official repositories. The current Code AArch64 candidate requires 130 official packages and 187,488,456 download bytes before cache reuse. A live Samsung gate renders that exact plan while keeping Install disabled and all 36 shared-root pacman entries unchanged.
