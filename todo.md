...
- [ ] Open the project in Code and validate editing, search, Git, terminal PTY, language services, restore/build, and extension-host subprocesses.
  ...
  - [ ] Report managed child executables rather than the sealed glibc loader to parent process inspection. Bash correctly has `Name: bash` and `--argv0 bash`, but Code labels its terminal with the content-addressed loader `.so` because cross-process `/proc/<pid>/exe` still exposes the loader.
