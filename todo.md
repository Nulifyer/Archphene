...
- [ ] Add capabilities in test-gated vertical slices; do not port code merely because it existed before.
  ...
  - [ ] Launcher wrapper generation and runtime-service binding
    ...
    - [ ] Remove repository-wide VCS/dirty-state metadata from the staged launcher-template bytes. Unchanged launcher sources currently produce a new template digest after unrelated manager commits or dirty-tree changes, causing needless user-confirmed wrapper updates.
