...
- [ ] Perform a complete UX pass over discovery, package details, installation, launcher creation, updates, storage, permissions, setup, settings, and recovery.
  ...
  - [ ] Add adaptive tablet and external-display layouts with a wider package workspace, persistent navigation, and productive keyboard/mouse density instead of stretching the phone layout.
  - [ ] Audit populated package results/details, every durable transaction phase, and an active terminal in the new navigation. Clean-data navigation was validated without downloading packages; do not claim those populated states until the existing package fixtures are deliberately restored and rerun.
