...
- [ ] Complete post-migration Bash validation across every documented build and test entry point.
  ...
  - [ ] Run the remaining standalone scripts not covered by the two broad regression entry points and confirm that each printed success claim is backed by an assertion. Hardware- and 16 KB-specific scripts remain tied to their named lanes.
    ...
    - [x] Restore KCalc/Mousepad menu, dialog, normalized-bound, semantic-input, and reverse-action workflows in `test-real-app-accessibility.sh`. Current-source emulator KCalc and Mousepad pass; the signer-bound Samsung KCalc passes, while its older Mousepad bridge fails honestly after recovery because the File semantic action is routed to Linux coordinate `1,1` instead of the Android-reported item bounds.
    - [x] Restore permission-resource selection, timestamped PipeWire frame consumption, denial/no-reprompt, lease release, and Linux-process cleanup in `test-generated-camera-app.sh`. The restored gate passes an official unmodified Snapshot wrapper on the current-source x86_64 emulator and the signer-bound AArch64 Samsung. It also found and fixed mismatched Java/native environment-entry bounds plus GTK4 preload leakage into the libc-only camera supervisor.
...
- [ ] Finish Qt and GTK appearance consistency.
  ...
  - [ ] Pass KCalc and Mousepad in light/dark and Auto/18/20/22 dp visible-control modes across phone, tablet, and docked layouts; repeat current-source core cases on physical AArch64.
    ...
    - [ ] Repeat current-source core cases on physical AArch64. The Samsung manager and Terminal are now current-source, but KCalc and Mousepad remain separately installed wrappers signed by the unavailable prototype wrapper key; replace and preserve each app sandbox deliberately before claiming parity.
      - The earlier Mousepad wrapper exposes a recovery-dialog viewport/presentation mismatch and a stale black host after the Linux process exits. Its real AccessibilityService flow also exports the File node at normal Android bounds but dispatches the reverse action to Linux coordinate `1,1`, so its menu never opens. Current-source emulator Mousepad passes the same workflows.
      - Signer replacement now checks Android's install-source consent before uninstalling the old wrapper, persists the pending migration across Settings, and asks for final destructive confirmation only after permission is granted. The physical Camera wrapper remained installed throughout the deny/allow/cancel regression.
...
- [ ] Execute the release-gate representatives in `docs/compatibility-matrix.md`.
  ...
  - [ ] Fix generic auxiliary-command runtime closure for generated wrappers. The current Foot wrappers run their verified primary Foot/Bash closure, but invoking the packaged `clear` helper fails because its `libc.so.6` dependency is absent from the helper's loader namespace. Resolve this in closure publication/materialization and add a package-agnostic subprocess test; do not patch Foot.
