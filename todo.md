...
- [ ] Add a bounded AUR workflow suitable for packages such as `visual-studio-code-bin`.
  ...
  - [ ] Provision a minimal verified Arch build root in the Builder's private storage, execute the approved build without Android network permission, and return verifiable package output through a manager-owned descriptor.
    - [ ] Before community execution is enabled, move reusable workspace mutation behind no-follow directory-FD/openat-style Rust operations, kill and reap every prior Builder descendant, and treat all Builder-writable state as hostile after each recipe.
      ...
      - [ ] Apply the same hostile-state rule to the disposable recipe/build/output workspace, retain and cancel/reap the active build process group, and re-run same-UID cleanup before every build attempt.
    - [ ] Report distinct download, closure staging, root scan/extraction, recipe build, and output verification progress. The current cold Samsung run remains on `Staging verified build package 152/152` for several minutes while the 1.2 GiB root is scanned and extracted.
