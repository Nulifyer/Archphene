...
- [ ] Finish Qt and GTK appearance consistency.
  ...
  - [x] Add display-aware Auto plus explicit 32, 40, and 48 dp control-density policy independently from geometry and text scaling; feed the selected minimum target into GTK metrics and the Qt style.
  - [x] Replace the appearance cycle-buttons with described, accessible discrete sliders for geometry, text, and control density; expose the actual 32/40/48 dp control targets, cover 100-200% text sizing, and make 32 dp the fresh-install control default while retaining display-aware Auto.
  ...
  - [x] Validate the public slider endpoints on current-source emulator wrappers: Mousepad changes live from 100%/32 dp to 200%/48 dp with stable Android and Linux PIDs, while KCalc launches with the rebuilt 42 pt Qt policy. Rebuild and checksum the Qt platform plugin for x86_64 and AArch64.
  ...
  - [ ] Pass KCalc and Mousepad in light/dark and Auto/32/40/48 dp modes across phone, tablet, and docked layouts; repeat current-source core cases on physical AArch64.
