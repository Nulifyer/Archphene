...
- [ ] Add a bounded AUR workflow suitable for packages such as `visual-studio-code-bin`.
  ...
  - [ ] Persist the verified built-package capability across manager process death, define recursive AUR dependency behavior, and finish the install-script/scriptlet policy.
    - [ ] Treat required sibling outputs from one split PKGBUILD as one reviewed build/install transaction. Exclude sibling names from the official pacman build closure, verify every required output and its dependency graph, and install the selected split closure atomically. The live `dotnet-sdk-bin` Samsung gate currently reaches this boundary after verifying its SDK source, then correctly exposes the four sibling names that official repositories cannot resolve.
