...
| `code` | Blocked after meaningful progress: the manager resolves, signature-verifies, extracts, compiles shared schemas/MIME data, and classifies the real 36-package closure. Runtime-pack publication rejects Code's absolute icon symlink into `/usr/lib/code`, exposing the broader absence of package-owned `/usr/lib/<app>` data and dependency-command publication (`electron42`). | Extend the generic verified pack format and symlink model, then test Electron sandbox/Ozone Wayland, multiprocess lifecycle, GPU, IME, project tree, terminal, file watching, dialogs, and extensions. Do not add a Code-specific bypass. |
