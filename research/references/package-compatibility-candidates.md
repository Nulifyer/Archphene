...
|---|---|---|
| `gnome-text-editor` | Validated on current-source x86_64 and manager-owned AArch64: generic SAF import/edit/writeback/cold reopen, DocumentsUI provider discovery, live/cold theme, semantic tabs/menus/Preferences, Android paste, non-Latin/emoji preedit and commit, Linux selection/copy, exact Android clipboard readback, graceful close, and prior session/clipboard restoration pass. | Keep it in the daily regression and broaden physical tablet/external-display endurance with the shared display lane. |
| `kate` | Partial: current-source x86_64 full closure installation and rendering pass after the runtime adopted daemonized GUI descendants. Stable-process tablet portrait/landscape and a real temporary 1920x1080 emulator display pass; display-targeted pointer/keyboard input creates text. A pre-existing physical AArch64 wrapper also maps Kate and accepts Android IME text. | Test tabs, split views, large documents, dialogs, sessions, secondary Linux windows, save/reopen, destructive lifecycle, and current-source AArch64 parity. |
| `foot` | Validated on current-source x86_64 emulator and AArch64 Samsung installs: signed closure review, three-launcher approval, stale-signer recovery, Android source permission, post-install ownership/broker/publication review, and bounded installed-ELF plus separately persisted process-map topology that both report Native Wayland on real launches. Readable density/theme behavior, UTF-8 preedit/commit, bidirectional clipboard and exact mouse selection, visible scrollback, live resize without process churn, graceful close, force-stop, and cold relaunch pass. The launch gate also fixed the generic GTK-settings preload's missing explicit GLib linkage for non-GTK processes. Generic `wev` and `wl-clipboard` suites cover hardware/modifier/repeat and protocol transfer on both devices. Unpublished Android PATH commands now fail closed instead of entering the wrong linker namespace. | Decide a bounded policy for optional dependency commands, then include Foot in sustained daily-use and external-display runs. |
| `code` | Partial on current-source x86_64 and manager-owned AArch64: the generic official/AUR pipelines publish one app-drawer launcher per Code entry while excluding dependency-owned Avahi entries. Stock Code reaches a crisp Ozone/Wayland workbench with persistent shared, watcher, extension-host, and Node workers. x86_64 additionally passes graceful close/relaunch and an integrated-terminal MVC restore/build/run workflow. Both ABIs pass extension-gallery search/install, native GTK file selection, YAML editing, Red Hat YAML activation/providers, Android DocumentsUI single/multiple Open, and Save As with the exact user-selected Linux-visible basename and byte-exact Android destination. The diagnostic `--disable-gpu` flag is no longer required, although presentation remains software. | Replace temporary reduced-sandbox flags with reviewed product policy, restore acceleration, and test C# language service/debugger/breakpoints, broader editing/clipboard/IME/Git behavior, tablet/external display, and sustained lifecycle on both architectures. |
| `gtk4-demos` | Resolution/signature probe passes. | Use only for GTK widget/rendering cases not already covered by Text Editor. |
| `supertux` | A pre-existing physical AArch64 wrapper maps and renders the title screen plus first-run network prompt. The current x86_64 closure now resolves, verifies, stages, publishes, and installs, then fails closed with `Failed loading SDL3 library`: verified `sdl3` is in the closure, but its `dlopen`-only library is omitted by `DT_NEEDED` pack reduction. | Add bounded package-generic discovery/retention for exact verified dynamic-library names, then test sustained rendering, audio focus, controls/pointer capture, pause/resume, and fullscreen. |
| `vulkan-tools` | Manager-provisioned unmodified `vulkaninfo` plus separately installed `vulkan-swrast` enumerate llvmpipe through immutable Terminal generations on x86_64 and physical AArch64. | Keep presentation as a deliberate frontier test until an Android-backed ICD or Venus path can drive `vkcube-wayland`; CPU enumeration is packaging evidence, not accelerated presentation. |
...
The physical Foot wrapper has since been deliberately replaced with a current-source AArch64 build after a verified backup and user-authorized signer migration. Other retained physical wrappers in this wave still use the earlier development signing lineage and remain controls rather than current-source parity evidence.
