...
| `foot` | Validated on current-source x86_64 emulator and AArch64 Samsung installs: signed closure review, three-launcher approval, stale-signer recovery, Android source permission, post-install ownership/broker/publication review, readable density/theme behavior, UTF-8 preedit/commit, bidirectional clipboard and exact mouse selection, visible scrollback, live resize without process churn, graceful close, force-stop, and cold relaunch pass. The launch gate also fixed the generic GTK-settings preload's missing explicit GLib linkage for non-GTK processes. Generic `wev` and `wl-clipboard` suites cover hardware/modifier/repeat and protocol transfer on both devices. Unpublished Android PATH commands now fail closed instead of entering the wrong linker namespace. | Derive package-generic toolkit/protocol topology, decide a bounded policy for optional dependency commands, then include Foot in sustained daily-use and external-display runs. |
