...
| `foot` | Validated on the current-source x86_64 emulator and AArch64 Samsung wrappers: readable density/theme behavior, UTF-8 preedit/commit, bidirectional clipboard and exact mouse selection, visible scrollback, live resize without process churn, graceful close, force-stop, and cold relaunch pass. Generic `wev` and `wl-clipboard` suites cover hardware/modifier/repeat and protocol transfer on both devices. Unpublished Android PATH commands now fail closed instead of entering the wrong linker namespace. | Decide a bounded generic policy for optional dependency commands, then include Foot in sustained daily-use and external-display runs. |
