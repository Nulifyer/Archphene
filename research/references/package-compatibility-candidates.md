...
| `foot` | Partial: current-source x86_64 maps a warning-free Bash PTY after Bash command publication, X11 locale staging, `C.UTF-8`, and monospace fallback fixes. Typed `echo` input produces output with stable processes. A pre-existing physical AArch64 wrapper also provides a working basic PTY. | Establish current-source AArch64 parity, then test Unicode/compose, hardware keyboard, modifiers/repeat, scrolling, selection, clipboard, resize, and destructive lifecycle. |
