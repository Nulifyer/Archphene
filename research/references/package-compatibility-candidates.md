...
| `foot` | Partial: current-source x86_64 maps a warning-free Bash PTY; phone/tablet/docked density gates, 42 px phone text, 126 px touch CSD, contrast, visible command output, and same-process Android light/dark switching pass. Generic Wayland input and clipboard protocol suites also pass. | Establish current-source AArch64 parity, then complete Foot-specific Unicode/compose, scrolling, selection, resize, and destructive lifecycle. |
