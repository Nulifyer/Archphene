...
before="$(package_version)"; archphene_adb_run push "$candidate" "$remote" >/dev/null; archphene_adb_run shell run-as "$package" cp "$remote" cache/archphene-rejected-update.apk; archphene_adb_run shell run-as "$package" chmod 600 cache/archphene-rejected-update.apk; hash="$(archphene_sha256_file "$candidate")"; rejected_update "$hash" 'Install failed: APK version downgrade rejected'; [[ "$(package_version)" == "$before" ]] || archphene_die "downgrade changed version"; bad_hash="$(printf '0%.0s' {1..64})"; [[ "$bad_hash" != "$hash" ]] || bad_hash="$(printf 'f%.0s' {1..64})"; rejected_update "$bad_hash" 'Install failed: APK SHA-256 mismatch'; [[ "$(package_version)" == "$before" ]] || archphene_die "checksum rejection changed version"; archphene_note "Manager update rejections passed on $serial: retained $before."
