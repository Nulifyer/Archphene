...
podman run --rm -v "$ARCHPHENE_ROOT:/workspace" -w /workspace "$image" \
  bash scripts/build-android-dbus.sh "$architecture"
archphene_require_file "$ARCHPHENE_ROOT/tooling/build/android-dbus/$architecture/dbus-daemon"
archphene_require_file "$ARCHPHENE_ROOT/tooling/build/android-dbus/$architecture/portal-service"
archphene_note "Android D-Bus runtime built for $architecture."
