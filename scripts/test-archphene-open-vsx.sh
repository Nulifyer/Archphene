...
archphene_wait_ui 'text="Command, e.g. btop"' "open-vsx-field-$serial" 15
archphene_tap_ui_pattern "$ARCHPHENE_UI" \
  'text="Command, e.g. btop"' 'Linux shell input'
archphene_adb_run shell input text \
  'bash%s/usr/bin/archphene-open-vsx-test' >/dev/null
archphene_adb_run shell input keyevent KEYCODE_BACK >/dev/null
archphene_wait_ui 'text="Send"' "open-vsx-send-$serial" 10
