...
try {
    ...
    podman run --rm -v "${Root}:/workspace" -w /workspace `
        ...
        bash scripts/build-secrets-capability-probe.sh
    if ($LASTEXITCODE -ne 0) { throw "Secrets capability probe build failed" }
} finally {
...
Write-Host "Container-built secrets capability probe: $apk"
