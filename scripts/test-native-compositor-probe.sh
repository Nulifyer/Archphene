...
key_sent=false; tap_sent=false; scroll_sent=false; touch_sent=false; deadline=$((SECONDS + 60)); completion='generation-checked JNI handles, registry, runtime fork/exec, Android bitmap, xdg toplevel, keyboard input, damage-batched buffer scale/transform, viewporter/fractional scaling, Choreographer-paced frames, MotionEvent pointer/wheel/touch input, compound pointer confinement, cursor surfaces, pointer gestures, nested popup grabs, synchronized subsurface trees, committed parent geometry, demand-driven clipboard, and Android InputConnection UTF-8 text-input v3 lifecycle complete'
