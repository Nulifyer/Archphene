...
fixture="$ARCHPHENE_ROOT/tests/fixtures/archphene-terminal-color-test"
device_temporary="/data/local/tmp/archphene-terminal-color-test-$serial"
installed_fixture=files/arch-root/usr/bin/archphene-terminal-color-test
if [[ -z "$apk" ]]; then
  apk="$ARCHPHENE_ROOT/android/app/build/outputs/apk/debug/app-debug.apk"
fi
output_dir="$ARCHPHENE_ROOT/tooling/build/terminal-colors"
mkdir -p "$output_dir"
...
archphene_adb_run shell run-as "$package" test -x files/arch-root/usr/bin/bash ||
  archphene_die "terminal color regression requires installed bash"
archphene_adb_run shell run-as "$package" test -x files/arch-root/usr/bin/tput ||
  archphene_die "terminal color regression requires installed tput"
