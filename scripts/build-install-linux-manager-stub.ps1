...
Run-Native { & (Join-Path $BuildTools "aapt2.exe") link -o (Join-Path $Out "unsigned.apk") -I (Join-Path $Sdk "platforms/android-36/android.jar") --manifest (Join-Path $App "AndroidManifest.xml") --java (Join-Path $Out "gen") -A $Assets (Join-Path $Out "compiled/res.zip") } "aapt2 link"
...
Run-Native { & $Adb install -r (Join-Path $Out "archpheneos-manager.apk") } "adb install"
Run-Native { & $Adb shell monkey -p org.archpheneos.manager -c android.intent.category.LAUNCHER 1 } "adb launch"
