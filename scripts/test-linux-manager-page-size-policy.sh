...
page4_serial=emulator-5554
page16_serial=emulator-5556
manager_apk="$ARCHPHENE_ROOT/prototypes/linux-app-manager-stub/out-linux/archphene.apk"
allow_apk_replacement=false
while (($#)); do
  case "$1" in
    --page-4k-serial) page4_serial="${2:?missing value for --page-4k-serial}"; shift 2 ;;
    --page-16k-serial) page16_serial="${2:?missing value for --page-16k-serial}"; shift 2 ;;
    --manager-apk) manager_apk="${2:?missing value for --manager-apk}"; shift 2 ;;
    --allow-apk-replacement) allow_apk_replacement=true; shift ;;
    -h|--help)
      echo "usage: $0 [--page-4k-serial SERIAL] [--page-16k-serial SERIAL] [--manager-apk PATH] --allow-apk-replacement"
      exit 0
      ;;
    *) archphene_die "unknown argument: $1" ;;
  esac
done
[[ "$allow_apk_replacement" == true ]] ||
  archphene_die \
    "--allow-apk-replacement is required because this legacy gate replaces the manager APK on two emulators"
archphene_require_file "$manager_apk"
manager_apk="$(realpath "$manager_apk")"
