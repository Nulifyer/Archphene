...
& $Adb shell am force-stop $KCalc | Out-Null
& $Adb shell monkey -p $Manager -c android.intent.category.LAUNCHER 1 | Out-Null
Start-Sleep -Seconds 2
& $Adb shell uiautomator dump /sdcard/archphene-manager-test.xml | Out-Null
$Ui = (& $Adb shell cat /sdcard/archphene-manager-test.xml) -join "`n"
...
$Processes = (& $Adb shell ps -A -o PID,PPID,NAME,ARGS) -join "`n"
