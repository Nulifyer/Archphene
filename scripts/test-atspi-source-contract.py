...
def main() -> None:
    ...
    lifecycle_tokens = (
        ...
        (translator_source, "if (cached_added == 0) {",
         "live-root retention during partial refresh"),
        (translator_source,
         "#define ROOT_MAX (MAX_APPLICATIONS + MAX_TRANSIENT_ROOTS",
         "bounded event-priority roots"),
        (translator_source, "+ MAX_CACHED_WINDOWS)",
         "bounded cached traversal roots"),
        ...
        (translator_source, "append_root_reference(applications, &count,",
         "cached window live traversal"),
        (translator_source, "archphene_atspi_tree_add_root(",
         "cached window fallback publication"),
