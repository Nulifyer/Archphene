...
while (($#)); do
  case "$1" in
    ...
    -h|--help)
      echo "usage: $0 --serial SERIAL --apk PATH --package CURRENT_LAUNCHER_PACKAGE"
      exit 0
      ;;
...
cleanup() {
  if [[ "$launcher_running" == false ]]; then
    archphene_adb_run shell am force-stop "$package" >/dev/null 2>&1 || true
  fi
  if [[ "$manager_running" == true && "$launcher_running" == false ]]; then
    archphene_adb_run shell am start -W -n "$manager_activity" >/dev/null 2>&1 || true
  elif [[ "$manager_running" == false && "$launcher_running" == false ]]; then
