...
try {
    ...
    podman run --rm -v "${Root}:/workspace" -w /workspace `
        ...
        bash scripts/build-accessibility-capability-probe.sh
    if ($LASTEXITCODE -ne 0) { throw "Accessibility capability probe build failed" }
} finally {
...
Write-Host "Container-built accessibility capability probe: $apk"
