...
$ArchRuntimeRoot = Join-Path $Root "tooling/downloads/arch-curated-kcalc-x86_64/runtime-root"
...
Run-Native { & (Join-Path $BuildTools "apksigner.bat") sign --ks $Key --ks-pass pass:android --key-pass pass:android --out (Join-Path $Out "archpheneos-kcalc.apk") (Join-Path $Out "aligned.apk") } "apksigner sign"
Run-Native { & (Join-Path $BuildTools "apksigner.bat") verify --verbose (Join-Path $Out "archpheneos-kcalc.apk") } "apksigner verify"

$Adb = Join-Path $Sdk "platform-tools/adb.exe"
if (-not $SkipInstall) {
    Run-Native { & $Adb install -r (Join-Path $Out "archpheneos-kcalc.apk") } "adb install"
    Run-Native { & $Adb shell am start -n org.archphene.linux.kcalc/org.archphene.linux.kcalc.MainActivity } "adb launch"
}
