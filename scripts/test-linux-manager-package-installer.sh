...
archphene_adb_run shell am start -f 0x20000000 -n "$manager/.MainActivity" --es archphene_test_apk_url "file:///data/user/0/$manager/cache/archpheneos-kcalc-update.apk" --es archphene_test_apk_sha256 "$hash" --es archphene_test_apk_package "$kcalc" >/dev/null
