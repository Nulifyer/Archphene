...
serial=emulator-5554; skip_document=false; skip_installer=false
while (($#)); do case "$1" in --serial) serial="${2:?}"; shift 2;; --skip-document-workflow) skip_document=true; shift;; --skip-package-installer) skip_installer=true; shift;; -h|--help) echo "usage: $0 [options]"; exit 0;; *) archphene_die "unknown argument: $1";; esac; done
...
for test in "${tests[@]}"; do start=$SECONDS; archphene_note "==> $test"; if [[ "$test" == native-compositor ]]; then "$ARCHPHENE_SCRIPTS_DIR/build-install-native-compositor-probe.sh" --serial "$serial" --android-abi x86_64; "$ARCHPHENE_SCRIPTS_DIR/test-native-compositor-probe.sh" --serial "$serial" --android-abi x86_64; else "$ARCHPHENE_SCRIPTS_DIR/$test.sh" --serial "$serial"; fi; archphene_note "PASS $test ($((SECONDS-start))s)"; done
