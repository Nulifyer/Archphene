...
static int decode_uri_response(
        int multiple, char *uris, size_t uri_stride, size_t max_uris,
        size_t *uri_count, char *response) {
    char *save = NULL;
...
int archphene_android_open_files(
    ...
    *uri_count = 0;
    ...
    int result = broker_request(request, response, response_size);
    if (result != 0) return result;
    return decode_uri_response(
            multiple, uris, uri_stride, max_uris, uri_count, response);
}
...
int archphene_android_open_directory(
    ...
    int length = snprintf(request, sizeof(request),
            "ARCHPHENE/4\tOPEN_DIRECTORY\t%s", encoded_title);
    if (length <= 0 || (size_t)length >= sizeof(request)) {
        errno = ENOSPC;
        return -1;
    }
    ...
    if (result != 0) return result;
    size_t count = 0;
    result = decode_uri_response(
            0, uri, uri_size, 1, &count, response);
    if (result == 0 && count != 1) {
        errno = EPROTO;
        return -1;
    }
