...
Wrappers whose verified ELF closure contains a CUPS client declare the exact V5 `printing` contract. The [XDG Print portal](https://flatpak.github.io/xdg-desktop-portal/docs/doc-org.freedesktop.portal.Print.html) accepts an already-rendered regular PDF descriptor, transfers it over the manager's authenticated same-UID socket and Binder session, validates the file type, `%PDF-` header, Android-parsed page structure, and a 256 MiB size limit, and stages it only in the wrapper's private cache. Android's `PrintManager` remains the policy and destination authority, including the system Save as PDF destination. Up to four print jobs may be active in one wrapper; completion or cancellation removes the private staging file, and stale files are removed when the wrapper next starts.
...
Wrappers whose verified ELF closure contains a Pulse client declare `audio-output,audio-input`. The manager starts a session-scoped PulseAudio native-protocol server, exports its private Unix socket only to the authenticated manager-owned Linux process through `PULSE_SERVER`, and renders a stereo 48 kHz sink through Android AAudio. OpenSL ES is the playback fallback when AAudio cannot initialize. The socket remains in manager-private cache storage and is never shared with another Linux session.
...
A generated wrapper declaring the exact V8 `secrets` contract receives a private encrypted collection. The manager's private portal validates and bounds metadata, passes secret bytes only through regular descriptors, and relays the operation over its already authenticated Binder session. The generated wrapper—not the shared manager UID—owns the collection and its Android Keystore namespace. Records are named by a SHA-256 digest of the identifier, encrypted in full with AES-256-GCM, authenticated against the record filename, and written atomically. The non-exportable key uses randomized encryption and unlocked-device enforcement where supported. The store is limited to 256 records, 64 KiB per secret, 8 KiB of attributes, and a 1 MiB metadata index. No Android runtime permission or accessibility-service privilege is involved.
...
The current Rust/Kotlin production path passes on the Android 16 x86_64 emulator and physical AArch64 Samsung. A state-preserving gate validates the direct capability contract, the real D-Bus wire client, and unmodified shared-root Arch `secret-tool`: create/read, search, update/replace, empty values, delete, persistence across launcher process death, precise oversized and missing-item failures, cross-UID denial, absence of unique plaintext values from Android logs, and restoration of the initial collection count. Repeated close/relaunch cycles also verify that stale compositor teardown cannot remove the replacement Wayland socket. Full-device captures show the unmodified Seahorse client rendered and usable in Android light and dark modes.

The earlier isolated compatibility fixture additionally covers the KWallet D-Bus API and `kwallet-query` on 4 KB x86_64 plus the patched compatibility `kwalletd6` on physical AArch64. That evidence remains useful client coverage, but it is not substituted for the current generated-wrapper gate. Upstream Arch x86_64 client closures remain blocked on 16 KB Android when their ELF files are only 4 KB-aligned.
