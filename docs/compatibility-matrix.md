...
| `qt5ct` | Qt 5 diagnostic | Partial | Clean x86_64 installation proves the manager automatically includes the signed dependency-owned `qt5-wayland` platform companion; a migrated Samsung root was repaired through the same UI. Stock Qt 5 then connects and renders through current generated app shells on both exact ABIs, publishing repeated bounded 24×24 legacy arrow cursor surfaces through Binder v7; Samsung additionally proves a pointing-hand hotspot, rapid shape changes, semantic tab actions, and stable-process portrait/landscape reflow whose visible OK/Cancel/Apply buttons retain aligned 43 px-high Android bounds. `qt5ct` intentionally expects its own platform-theme setting and is cursor/runtime evidence, not a general Qt 5 appearance claim. |
| `mousepad` | GTK 3 | Partial | Current-source x86_64 document, IME, touch, popup, secondary-window, accessibility, live-theme, content-geometry, and phone/tablet/docked density gates pass. The current-source Samsung app shell repeats accessibility, IME/touch, Preferences checkbox/close interaction, aligned 80×73 px semantic Close targets, live theme, Material You checked-state pixels, host cleanup, and document restart/conflict/writeback. Protocol v21 additionally keeps a second unmodified Mousepad window in a separate bounded Android document task on a live 720 dp adaptive viewport, restores both task identities after app-shell process death, and retains the same manager-owned Linux process. The normal 420-density phone viewport retains one compact task with a bounded two-window switcher; a live 420→240→420 policy transition created and released the child Android task while retaining Linux PID 22126 and both Linux toplevels. With both tasks present, exact `ACTION_VIEW`, `ACTION_EDIT`, and `ACTION_SEND` replacement passes; the edit lane writes the changed manager-owned file back to its original granted Android URI. Multi-task notifications, root-first close, and force-stop grace also pass on Samsung. The same generated app shell passes the XDG folder-portal contract, exact recursive import, collision, cancellation, and full-device visual gates on both ABIs; an installed GTK application with its own folder-selection workflow remains pending. Adwaita owns complete base widget states; x86_64 multi-task parity, isolated manager-absence/removal, and physical external-display coverage remain. |
...
| `foot` | Native Wayland terminal | Validated | Current-source x86_64 emulator and AArch64 Samsung installs pass signed closure review, user approval for three app shells, stale-signer recovery, one-time Android source permission, executable/broker/publication review, and clean app-drawer launch. The bounded installed-ELF dependency review and separately persisted process-map observation both report Native Wayland on real launches; the UI retains app-shell coverage and static/observed provenance. The wrappers also pass clean Bash PTY launch, readable density-aware light/dark visuals, UTF-8 preedit/commit, bidirectional clipboard, exact mouse selection, visible scrollback, live resize with stable processes, graceful close, force-stop, and cold relaunch. The dual-ABI launch gate caught and fixed a generic GTK-settings preload that previously assumed GLib symbols existed in a native-Wayland process. Generic `wev` covers physical hardware modifiers/repeat. Unpublished host commands such as `clear` fail closed; optional dependency-command publication is not part of this Foot claim. |
| `supertux` | SDL/Wayland | Validated | Current manager-generated x86_64 and AArch64 wrappers retain the bounded verified runtime-loaded SDL3/graphics/audio closure and pass accelerated animated rendering, real contributed-level movement/jump, PulseAudio playback with serialized sink suspension and Android focus abandon/reacquire, keyboard navigation, generic finger pointer/button delivery, HOME/resume, live tablet resize, explicit fullscreen→windowed transitions, and stable process checks. Binder protocol v19 distinguishes absent, explicit-empty, and strong editor evidence, so the generated app shell prevents an implicit SDL gameplay IME while preserving touched empty editors and explicit long-press requests. It also applies automatic landscape-sensor presentation, maps input through the compositor's logical extent, exposes requested fullscreen/maximized state in presentation snapshots, and restores exact prior state. Full-device evidence passes on the emulator and Samsung. Upstream 0.7.0 never requests SDL relative mode, mouse grab, capture, or pointer lock, so pointer capture is not part of the SuperTux claim and remains assigned to a separate SDL representative. |
...
| SDL | `supertux` plus a relative-mode application | Sustained rendering, audio focus, controller/pointer capture, pause/resume, and fullscreen. | Partial: SuperTux is validated on current-source x86_64 and AArch64 for real gameplay, generic pointer/button input, keyboard controls without an implicit IME, Pulse playback and focus handoff, Home/resume, explicit fullscreen→windowed transitions, live tablet resize, stable processes, and exact state restoration. Upstream SuperTux 0.7.0 never requests SDL relative mode, mouse grab, capture, or pointer lock. Archphene's generic relative/locked/confined-pointer protocol path passes independently, but an unmodified SDL application that explicitly requests capture remains required for this release lane. |
| Electron | `code` | Multiprocess runtime, PTY, file watching, project tree, IME, GPU, dialogs, and extension host. | Partial on current-source x86_64 and physical AArch64: stock Code reaches a stable Ozone/Wayland workbench with shared/watcher/extension-host services and a job-control-capable integrated Bash. Phone/IME resize is crisp, full-device close/relaunch passes, and x86_64 completes the .NET MVC terminal workflow. Private-root DNS/TLS, extension search/install, native GTK file selection, YAML editing, and Red Hat YAML activation/providers pass on both ABIs through the shared runtime. Both ABIs pass live Light→Dark→Light appearance with stable processes, exact profile restoration, measured full-surface changes, and inspected full-device frames. The current gate uses `--no-sandbox --disable-dev-shm-usage`; `--disable-gpu` is no longer needed, although Chromium still reports software GL. Generic sandbox policy, acceleration, broader editor/clipboard/lifecycle behavior, and the C# debugger remain. |
...
**Bridge eligible**: package-specific runtime and app-shell workflows still
