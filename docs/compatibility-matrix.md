...
| `supertux` | SDL/Wayland | Partial | A pre-existing, differently signed physical AArch64 wrapper maps and renders the title screen and first-run network prompt. The current x86_64 closure resolves, verifies, stages, publishes, and installs after Android trims clearable emulator caches, then fails closed at launch because dynamically loaded `libSDL3.so.0` is not retained by the `DT_NEEDED`-driven runtime-pack reduction. Generic verified `dlopen` library discovery must precede interaction testing. |
