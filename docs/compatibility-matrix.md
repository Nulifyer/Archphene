...
| `gnome-text-editor` | GTK 4/libadwaita | Partial | The current-source complete x86_64 closure installs and renders on the API 36 emulator. Android IME entry, a native popup, rotation, and warm resume preserve the draft and Linux process. A pre-existing, differently signed physical AArch64 wrapper also launches and cold-reopens an existing document. Current-source AArch64 parity, open/save/writeback, bidirectional clipboard, complex composition, and accessibility remain. |
| `kate` | Qt 6/KDE | Partial | The current-source complete x86_64 closure installs and reaches Wayland after auxiliary shebang materialization was fixed, but its primary exits `0` before mapping while children remain. A pre-existing, differently signed physical AArch64 wrapper maps the full Kate UI and accepts Android IME text without restarting. The build/lane divergence and process handoff must be reconciled before the interaction suite. |
| `foot` | Native Wayland terminal | Partial | The current-source complete x86_64 wrapper maps and renders, but `/bin/sh` cannot resolve `libc.so.6` and locale/font data are absent. A pre-existing, differently signed physical AArch64 wrapper maps a working `sh` PTY and accepts a typed `echo` command without restarting. Current-source AArch64 parity and the full Unicode, keyboard, scroll, selection, clipboard, and lifecycle suite remain. |
...
| Qt complex UI | `kate` | Tabs, split views, sessions, dialogs, secondary windows, and large text. | Partial: current x86_64 reaches Wayland but exits before mapping; an older physical AArch64 build maps and accepts IME input. Resolve the process/build divergence before broader workflows. |
| Native Wayland terminal | `foot` | PTY, Unicode, hardware keyboard, scrolling, selection, and clipboard. | Partial: current x86_64 maps but child-shell linkage and runtime data fail; an older physical AArch64 build provides a working basic PTY. |
