...
|---|---|---|---|
| `kcalc` | Qt 6/KDE | Validated | Manager resolve/verify/wrap/sign/install/update/uninstall, app-drawer launch, input, menus, scaling, secondary windows, accessibility, and process cleanup on x86_64. Current-source x86_64 passes live system light/dark, manager light/dark override, and Material You pixel/config checks; an installed AArch64 wrapper passes same-process bidirectional Android light/dark palette switching. |
| `mousepad` | GTK 3 | Validated | Menus, dialogs, IME, Android document import/edit/save/writeback/cold reopen, conflict preservation, and DocumentsProvider access on x86_64 and physical AArch64. Current-source x86_64 passes live system light/dark, manager override, and Material You pixel/config checks. |
...
| `foot` | Native Wayland terminal | Partial | The current-source complete x86_64 wrapper maps a clean Bash PTY after verified Bash publication, `C.UTF-8`/X11 locale staging, and monospace fallback fixes. Typed `echo` input produces output and returns to the prompt without Android or Linux process churn. A pre-existing, differently signed physical AArch64 wrapper also maps a working `sh` PTY. Current-source AArch64 parity and Unicode/compose, hardware keyboard, modifiers/repeat, scroll, selection, clipboard, resize, and destructive-lifecycle coverage remain. |
...
| Native Wayland terminal | `foot` | PTY, Unicode, hardware keyboard, scrolling, selection, and clipboard. | Partial: current x86_64 maps a warning-free Bash PTY and executes typed input; an older physical AArch64 build provides a working basic PTY. Full terminal interaction/lifecycle and current-source AArch64 coverage remain. |
