...
| `foot` | Native Wayland terminal | Validated | Current-source x86_64 emulator and AArch64 Samsung wrappers pass clean Bash PTY launch, readable density-aware light/dark visuals, UTF-8 preedit/commit, bidirectional clipboard, exact mouse selection, visible scrollback, live resize with stable processes, graceful close, force-stop, and cold relaunch. Generic `wev` covers physical hardware modifiers/repeat. The separate auxiliary `clear` command currently exposes a generic loader-closure defect and is not counted as a Foot failure. |
