...
| `mousepad` | GTK 3 | Partial | Current-source x86_64 document, IME, touch, popup, secondary-window, accessibility, live-theme, content-geometry, and phone/tablet/docked density gates pass. The current-source Samsung app shell repeats accessibility, IME/touch, Preferences checkbox/close interaction, aligned 80×73 px semantic Close targets, live theme, Material You checked-state pixels, host cleanup, and document restart/conflict/writeback. Protocol v20 additionally keeps a second unmodified Mousepad window in a separate bounded Android document task on a live 720 dp adaptive viewport, restores both task identities after app-shell process death, and retains the same manager-owned Linux process. The normal 420-density phone viewport retains one compact task with a bounded two-window switcher; a live 420→240→420 policy transition created and released the child Android task while retaining Linux PID 22126 and both Linux toplevels. The same generated app shell passes the XDG folder-portal contract, exact recursive import, collision, cancellation, and full-device visual gates on both ABIs; an installed GTK application with its own folder-selection workflow remains pending. Adwaita owns complete base widget states; x86_64 multi-task parity and physical external-display coverage remain. |
