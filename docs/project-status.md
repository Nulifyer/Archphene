...
1. **Shared native compositor**
   - continue after the validated registry, SHM, surface, initial xdg-toplevel, map/unmap, xdg_positioner, initial popup role/configure/destruction, pointer, XKB keymap, focus, and hardware-key slices with popup grabs/reposition/composition/dismissal, clipboard, Android IME/text input, and output state;
