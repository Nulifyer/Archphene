...
| Native compositor bootstrap | Rust wayland-server core cross-compiles for Android x86_64 and AArch64; registry/compositor/SHM discovery, socket and SCM_RIGHTS FD transfer, checked SHM pool/buffer creation, pixel-byte reads, resource destruction, and wl_surface create/destroy pass on both |
...
1. **Shared native compositor**
   - migrate registry/globals, SHM, surfaces, xdg-shell, seats, popups, clipboard, text input, and output state in protocol-order slices;
