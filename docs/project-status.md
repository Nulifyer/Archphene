...
1. **Shared native compositor**
   - continue after the validated registry, SHM, surface, initial xdg-toplevel, map/unmap, xdg_positioner, nested popup grab/configure/dismissal, pointer, XKB keymap, focus, hardware-key, and live output mode/scale slices with clipboard, Android IME/text input, and commit-gated popup configure application;
