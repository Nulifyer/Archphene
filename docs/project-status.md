...
| Shared runtime module proof | Manager-owned, hash-verified x86_64 modules are denied to an ungranted KCalc UID; explicit grants then execute both a static ELF and a dynamic program with shared patched loader/libc descriptors under that distinct UID without wrapper copies |
...
3. **Runtime storage**
   - expand the validated static and three-module glibc descriptor launches into signed, atomic, content-addressed runtime packs with generated catalogs;
