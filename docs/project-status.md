...
| Shared runtime modules | Linux and Windows builds generate a bounded APK-signed module catalog; parser rejection cases pass, ungranted wrapper access is denied, and explicit grants execute static, patched-glibc, and separate `DT_NEEDED` dependency descriptors under that distinct UID without wrapper copies |
...
3. **Runtime storage**
   - derive bounded descriptor catalogs from arbitrary verified dependency graphs and activate them as signed, atomic, content-addressed runtime packs;
