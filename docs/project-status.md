...
details. The fixed V4 core contract remains unchanged. Printing is the first
optional broker promoted from evidence to authority: only a verified CUPS-linked
launcher receives the exact V5 `printing` contract, while other optional bits
still do not enter wrapper metadata. Binder protocol v11 carries one bounded PDF
descriptor from the manager-owned portal to the authenticated visible wrapper;
the wrapper rechecks file type and size, reparses page structure, stages the
document privately, and invokes Android PrintManager. Workspace tests and
warning-denied Clippy pass. The real Samsung XDG PreparePrint/Print gate renders
one page, exposes Save as PDF, cleans cancellation state, and rejects malformed
PDF and pipe descriptors. Samsung Mousepad and emulator Code OSS remain
correctly incapable because their verified closures contain no CUPS evidence.
...
| Android capability broker | The current Rust/Kotlin manager publishes a V4 core wrapper contract for Wayland, input, IME, clipboard, documents, OpenURI, and notifications, plus an exact V5 printing contract only for verified CUPS-linked launchers. One randomized same-UID socket serves each authenticated visible launcher's private XDG portal. OpenURI accepts bounded HTTP(S) only, notifications remain wrapper-attributed, and Binder protocol v11 carries the implemented callbacks. The same boundary binds registry-derived desktop MIME declarations into each signed wrapper without exposing Android grants or paths to Linux. Repeatable exact-ABI gates cover browser policy, notifications, document intake, unsupported MIME, and optional-capability denial on x86_64 and physical AArch64; the physical device also passes the complete printing path. |
