...
audit is executable and fail-closed.
...
workflow creates a GitHub draft. All 14 records are complete. Deterministic release
...
build environment. Arch runtime transactions intentionally test current signed
repositories, and GTK compatibility uses checksummed maintained artifacts rather
than requiring historical reconstruction. `release-native-audit.py
--require-complete` now passes while preserving a fail-closed boundary for new
...
| Integrated Terminal | The manager owns the foreground-service PTY, shell process group, terminal parser, scrollback, selection, clipboard, rendering buffers, and teardown under its ordinary Android UID. It uses the same shared Arch root and home as graphical applications; CLI/TUI packages do not create app-shell APKs. Verified Bash selection, `C.UTF-8`, Unicode IME, hardware input, selection, bracketed paste, scrollback, resize, protocol replies, font scaling, Home/resume, graceful stop, process reaping, scoped logs, and full-device rendering pass on the API 36 x86_64 emulator and physical AArch64 Samsung. The 16 KB x86_64 device retains the explicit upstream-package compatibility boundary. |
| Project folders | A user-selected SAF tree receives a persisted scoped grant and an atomic `$HOME/Projects/<alias>` mirror through the manager Files page. Recursive initial copy, exact SHA-256 baselines, no-op sync, bidirectional file/directory additions, edits, and deletions, serial crash-safe multi-delete checkpoints, deterministic conflict copies, live action-only directional progress, durable exact conflict history/Retry, native long-hash cancellation, bounded SAF deadlines with fail-closed mutation timeout recovery, restart/disconnect retention, and forced deletion/replacement journal recovery pass on physical AArch64 and exact x86_64. Terminal and graphical tools use the same private POSIX mirror. |
...
Local debug builds can remain multi-ABI. Release builds emit separate x86_64 and
arm64-v8a manager and hidden Builder APKs signed with the same production
identity. The manager's integrated Terminal, package runtime, trust data, native
components, and generated app-shell template contain only the selected ABI. Both
manager variants launch the package runtime on matching devices, and the ARM
manager has generated, installed, and launched a real KCalc app shell on the
Samsung test device.
