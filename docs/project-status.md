...
| Android capability broker | The current Rust/Kotlin manager publishes a V3 wrapper contract for Wayland, input, IME, clipboard, documents, and OpenURI. One randomized same-UID socket serves each authenticated visible launcher's private XDG portal. OpenURI accepts bounded HTTP(S) only, rejects file/content/custom schemes and malformed authority data, crosses Binder protocol v8 without a URI grant, and starts a browsable Android intent from the wrapper Activity. Repeatable exact-ABI gates reject `file://` and visibly open Chrome on x86_64 and Brave on physical AArch64. A stock .NET 10 minimal web app running from Samsung's shared Arch root returns `Hello World!` through Android loopback, proving the VS Code development path's browser boundary. |
