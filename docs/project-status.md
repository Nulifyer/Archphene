...
1. **Shared native compositor**
   - move the validated native protocol core, demand-driven clipboard broker, and Android InputConnection view from the probe into one shared Activity/service runtime;
   - replace both application-specific Java compositor forks only after cross-toolkit regression gates pass;
   - keep JNI dispatch serialized on the compositor service thread and keep clipboard reads focused and client-requested.
