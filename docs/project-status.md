...
| Shared runtime modules | Linux and Windows builds generate a bounded APK-signed module catalog; parser rejection cases pass, ungranted KCalc access is denied, and explicit grants execute static and patched-glibc dynamic modules under that distinct UID without wrapper copies |
...
3. **Runtime storage**
   - expand the validated generated catalog and three-module glibc descriptor launch into signed, atomic, content-addressed runtime packs derived from arbitrary verified dependency graphs;
   - generalize the proven loader/libc symlink view to arbitrary verified transitive shared libraries;
