...
| Audio output | Verified Pulse evidence selects the exact `audio-output` launcher contract. A session-scoped manager-owned Pulse server renders through AAudio with OpenSL ES fallback; only the authorized manager-owned Linux process receives its private native-protocol socket. The manager requests no `RECORD_AUDIO`, Bionic audio dependencies cannot contaminate glibc application loading, and an active launcher without the capability is denied. Unmodified `pavucontrol`, bounded 48 kHz stereo playback, cleanup, fatal-log checks, and inspected full-device light/dark presentation pass on the x86_64 emulator and physical AArch64. Microphone input remains unported in the current replacement. |
