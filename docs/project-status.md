...
Durable conflict history remains. The complete matrix passes on the exact
x86_64 emulator build as well.
...
| Terminal project folders | A user-selected SAF tree receives a persisted scoped grant and an atomic `$HOME/Projects/<alias>` mirror. Recursive initial copy, exact SHA-256 baselines, no-op sync, bidirectional file/directory additions, edits, and deletions, serial crash-safe multi-delete checkpoints, deterministic conflict copies, native long-hash cancellation, bounded SAF deadlines with fail-closed mutation timeout recovery, restart/disconnect retention, and forced deletion/replacement journal recovery pass on physical AArch64 and exact x86_64. Durable conflict history and Terminal command exposure remain. |
