...
and full-device frames. The follow-up regular-file synchronizer is described
below; richer durable progress and conflict history remain.
...
removes the committed backup without resurrecting data. Directory deletion,
multi-delete batching, native cancellation during long hashes, provider
deadlines, and durable conflict history remain. The complete regular-file and
forced-death gate also passes on the exact x86_64 emulator build.
...
| Terminal project folders | A user-selected SAF tree receives a persisted scoped grant and an atomic `$HOME/Projects/<alias>` mirror. Recursive initial copy, exact SHA-256 baselines, no-op sync, bidirectional regular-file additions/edits/deletes, deterministic conflict copies, restart/disconnect retention, and forced pre/post-commit journal recovery pass on physical AArch64 and exact x86_64. Directory deletion, multi-delete batching, long-hash cancellation/provider deadlines, and Terminal command exposure remain. |
