...
| Android capability broker | The current Rust/Kotlin manager publishes a V4 wrapper contract for Wayland, input, IME, clipboard, documents, OpenURI, and notifications. One randomized same-UID socket serves each authenticated visible launcher's private XDG portal. OpenURI accepts bounded HTTP(S) only, rejects file/content/custom schemes and malformed authority data, and starts a browsable Android intent from the wrapper Activity. Standard portal/classic notifications cross Binder protocol v10 into the authenticated wrapper, which owns first-use consent, Android attribution, content intents, and withdrawal. The same authenticated boundary binds registry-derived desktop MIME declarations into each signed wrapper: matching `content://` VIEW/SEND streams are copied through SAF into `~/Documents/Android` before desktop-entry argument expansion, while grants and Android paths remain outside Linux. Repeatable exact-ABI gates cover browser policy, wrapper-attributed notifications, stock Mousepad text intake, Code OSS workspace intake, and unsupported-MIME rejection on x86_64 and physical AArch64. A stock .NET 10 minimal web app running from Samsung's shared Arch root returns `Hello World!` through Android loopback, proving the VS Code development path's browser boundary. |
