...
- The current manager does not yet perform complete on-device Arch dependency resolution, package-signature verification, wrapper generation, and signing.
- Current wrapper runtimes are mutable app-private extractions and need atomic, read-only materialization.
